...
Status diambil dari **plugin opencode** (`.opencode/plugins/agent-status.ts`, project-level) yang menulis file status per-PID di `%APPDATA%/poni-deck/agents/{pid}.json`. Plugin menulis ulang file via **heartbeat periodik 10 detik** sehingga file tetap fresh (di bawah TTL) selama proses opencode hidup. Monitor membaca file tersebut per polling dan menampilkannya di overlay (warna dot, teks compact bar, badge per agent) serta tray tooltip.
...
- Tray tooltip menampilkan ringkasan status aggregate, contoh: `"Poni Deck — 2 agents · 1 working"`.
...
- [ ] **Plugin opencode** — `.opencode/plugins/agent-status.ts`: subscribe event opencode, map event → status, tulis file status per-PID ke `%APPDATA%/poni-deck/agents/{pid}.json`.
- [ ] **Rust reader modul** — `poni-deck/src-tauri/src/status_reader.rs`: baca & parse file status per PID (serde), TTL 30 detik via file mtime.
...
- [ ] **Tray tooltip ringkasan** — contoh `"Poni Deck — 2 agents · 1 working"`.
...
Plugin agent-status.ts ──debounce 250ms──▶ write atomic
  ...
  │              %APPDATA%/poni-deck/agents/{pid}.json
  ▼                                              │
  ┌──────────────────────────────┐               │
...
`writeStatus(status)` = debounce 250 ms + atomic write Windows/Bun (tmp → `fs.rmSync(target, {force:true})` → rename) + heartbeat 10 detik, dengan target `{APPDATA}/poni-deck/agents/{process.pid}.json` (lihat detail di bawah).
...
**Status set yang ditampilkan:** `working` / `idle` / `waiting_confirmation` / `error` + fallback `running` jika file status tidak ada / stale (TTL 30 detik) / tidak valid. Set ini **berbeda** dari `Agent.status` di `planning/poni-deck.md:145` (`'running' | 'waiting_confirmation' | 'error' | 'done' | 'idle'`) (PQ13): v2 plan memakai status life-cycle agent (termasuk `done` = agent selesai), sedangkan plan ini menambahkan `working` sebagai status aktivitas real-time dan tidak memakai `done` — agent yang selesai kembali ke `idle`. Status `idle`, `waiting_confirmation`, `error`, `running` konsisten di kedua plan.
...
- **Tray tooltip** — SATU format final untuk semua kasus (dengan & tanpa plugin) (PQ10):
  `"Poni Deck — {N} agents · {summary}"`
  - `N` = jumlah total agent.
  - `summary`:
    - 0 agent → `idle` → `"Poni Deck — 0 agents · idle"`.
    - Semua agent berstatus sama → nama status (misal `"Poni Deck — 2 agents · working"`; tanpa plugin semua `running` → `"Poni Deck — N agents · running"`).
    - Campuran → `{jumlah} {status prioritas tertinggi}` (contoh `"Poni Deck — 2 agents · 1 working"`).
- Tidak ada notifikasi OS.

**Rust side:**

- `status_reader.rs` (modul baru) — `read(pid) -> Option<String>`: baca `{config_dir}/agents/{pid}.json`, parse serde, return status valid. **TTL 30 detik via file mtime** (`std::fs::metadata(path).modified()`): umur file > 30 detik → anggap tidak ada. Field `timestamp` hanya info/debug, tidak dipakai untuk TTL. Gunakan `config_dir()` dari `config.rs` (`%APPDATA%/poni-deck/`) — visibilitas `config_dir()` diubah menjadi `pub(crate)` agar bisa dipanggil dari modul ini. Tanpa dependency baru.
...
| R7: Plugin gagal baca env `APPDATA` | Rendah | Sedang | Fallback ke lokasi config default (`USERPROFILE/.config/poni-deck/agents/`), sama dengan logika `config_dir()` di `config.rs`. |
...
- [ ] **EC1: Plugin tidak terpasang** — scan menemukan `opencode.exe` tapi tidak ada file status → status `running`, dot hijau statis, tooltip `"Poni Deck — N agents · running"` (format unifikasi PQ10). Identik dengan behavior sebelum fitur (tanpa regresi status).
...
| Dependency | Tujuan |
|------------|--------|
| `config_dir()` di `config.rs` (`%APPDATA%/poni-deck/`) — visibilitas diubah `fn` → `pub(crate)` | Basis path `agents/` folder — plugin & Rust wajib konsisten memakai path ini; dipanggil oleh `status_reader.rs` |
...
- [ ] **T1: Plugin opencode `agent-status.ts`** — buat `.opencode/plugins/agent-status.ts` (project-level root repo) mengikuti pola "Struktur plugin (pseudo-code)" di §3 dengan **pola ekspor yang BENAR (Q21):** `export const agentStatus: Plugin = async (ctx) => ({ ... })` — fungsi async yang menerima `ctx` dan return objek hooks ber-tipe `Hooks` (contoh lengkap di §3); **BUKAN** `export const agentStatus: Hooks = { ... }` — loader opencode menolak ekspor non-fungsi (`TypeError("Plugin export is not a function")`, di-swallow) → plugin tidak pernah dimuat → seluruh fitur mati. **SATU hook `event` generik + `switch (event.type)`** untuk semua event bus, plus named hooks `tool.execute.before`/`tool.execute.after`. Detail implementasi:
  - **Init:** buat dir `{APPDATA}/poni-deck/agents/` (fallback `USERPROFILE/.config/...` jika `APPDATA` tidak ada), tulis file status awal `idle` untuk PID sendiri (`process.pid`).
...
- [ ] **T5: Rust `tray.rs`** — ubah `update_tray_tooltip` menerima ringkasan status dan merender format final tunggal (PQ10): `"Poni Deck — {N} agents · {summary}"` — 0 agent → `"Poni Deck — 0 agents · idle"`; semua status sama → nama status (misal `"Poni Deck — 2 agents · working"`, `"... · running"` tanpa plugin); campuran → `"Poni Deck — 2 agents · 1 working"`. Update default tooltip di `create_tray`. [S]
...
- Testing/verifikasi: `cargo check` dan `cargo build` di `poni-deck/src-tauri`.
...
- [x] **Q2: Path file status?** — **Keputusan final: `%APPDATA%/poni-deck/agents/{pid}.json`** (konsisten dengan `config_dir()` di `config.rs`). Di mesin ini: `C:\Users\ikade\AppData\Roaming\poni-deck\agents\{pid}.json`.
...
- [x] **Q12: Format tooltip final untuk semua kasus (dengan & tanpa plugin)?** — **Keputusan final (PQ10):** `"Poni Deck — {N} agents · {summary}"`; 0 agent → `summary` = `idle`; semua sama → nama status; campuran → `{jumlah} {status prioritas}`; tanpa plugin → `"... · running"`.
...
- [ ] **AC6:** Tray tooltip menampilkan ringkasan status aggregate, contoh `"Poni Deck — 2 agents · 1 working"`.
- [ ] **AC7:** Tanpa plugin → behavior seperti sebelumnya: status `"running"`, dot hijau statis, tooltip `"Poni Deck — N agents · running"` (format unifikasi PQ10, bukan `"N agent(s) running"`). Tidak ada regresi status & perilaku.
...
- [ ] **AC11:** `cargo check` dan `cargo build` di `poni-deck/src-tauri` berhasil tanpa error dan tanpa dependency baru.
...
- [Plan referensi: `planning/poni-deck.md`](poni-deck.md)
- [Plan referensi: `planning/poni-deck-phase1.md`](poni-deck-phase1.md)
- [config.rs `config_dir()`](file:///D:/dev/experiment-poni-agent/poni-deck/src-tauri/src/config.rs)
- [state.rs `AgentInfo.state`](file:///D:/dev/experiment-poni-agent/poni-deck/src-tauri/src/state.rs)
- [process_scanner.rs `scan_agents()`](file:///D:/dev/experiment-poni-agent/poni-deck/src-tauri/src/process_scanner.rs)
...
| `1.2.0` | `2026-07-31` | Planner | Resolve review putaran 2 PQ8–PQ13 + minor: mapping `session.status` via `status.type` (`busy`→`working`, `idle`→`idle`, `retry`→`working`), hapus semua referensi nilai `"running"` pada `session.status` (PQ8); hapus handler `session.updated` sebagai sumber status — `Session` tanpa field status (PQ9); unifikasi format tooltip `"Poni Deck — {N} agents · {summary}"` untuk semua kasus, termasuk 0 agent & tanpa-plugin (PQ10); definisi eksplisit "atomic write" = tanpa partial/corrupt read + konsistensi R3 (PQ11); `permission.asked` via generic `event` hook + `permission.replied` → `working` (PQ12); koreksi klaim selaras `poni-deck.md`, hapus A7 duplikat (PQ13); pindahkan Asumsi ke `### Asumsi` di §6, tambah EC17/EC18, AC15/AC16, Q8–Q12, referensi types.gen.ts & issue #9229. |
